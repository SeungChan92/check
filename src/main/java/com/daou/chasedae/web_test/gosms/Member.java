...
public class Member extends Category {
	...
	public void login(String id, String pw) throws Exception {
		By input_id = By.name("userid");
		By input_pw = By.name("password");
		By button_login = By.xpath("//a[@tabindex='30']");
		By button_logout = By.xpath("//a[@href='PPLoginMgr.qri?act=logout']");
		
		driver.get(baseUrl + "/");
		tool.wait.until(ExpectedConditions.visibilityOf(driver.findElement(input_id)));
		tool.wait.until(ExpectedConditions.visibilityOf(driver.findElement(input_pw)));
		driver.findElement(input_id).clear();
		driver.findElement(input_id).sendKeys(id);
		driver.findElement(input_pw).clear();
		driver.findElement(input_pw).sendKeys(pw);
		// 비밀번호 입력 안될 시 사용
		/*
		while(true)
		{
		 ...
		 */
		tool.wait.until(ExpectedConditions.attributeToBeNotEmpty(driver.findElement(input_pw), "value"));
		driver.findElement(button_login).click();
		tool.wait.until(ExpectedConditions.visibilityOf(driver.findElement(button_logout)));
		Thread.sleep(2000);
	}
